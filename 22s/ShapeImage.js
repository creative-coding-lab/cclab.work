class ShapeImage {
  constructor(size) {
    const number = floor(random(3, 7));
    const radDist = size * 0.45;
    this.points = this.generatePoints(radDist, number);
  ...
  generatePoints(radDist, number) {
    let points = [];
    for (let i = 0; i < number; i++) {
      let angle = (TWO_PI * i) / number + random(-1, 1) * 0.3;
      let x = cos(angle) * (radDist * random(0.6, 1.0));
      let y = sin(angle) * (radDist * random(0.6, 1.0));
  ...
  generateGraphics(w, h) {
    ...
    g.noStroke();
    g.fill(random(210), random(210), random(210), 50);
    g.beginShape();
