...
function draw() {
  ...
  if (pose) {
    ...
    // calculate midpoint (average) between eyes
    let midEyeX = (eyeLX + eyeRX) / 2;
    let midEyeY = (eyeLY + eyeRY) / 2;

    // get differences between midpoint and nose
    let dx = noseX - midEyeX;
    let dy = noseY - midEyeY;
    ...
    // adjust the range to 0–360 if you prefer
    if (angle < 0) {
      angle += 360;
    }
    angle -= 90;
    ...
    // draw the line from midpoint to nose
    stroke(255, 0, 255);
    line(midEyeX, midEyeY, noseX, noseY);

    // display the angle
    noStroke();
    fill(0, 255, 0);
    textSize(16);
    text("Look-at angle: " + round(angle) + "°", 20, 30);
  }
}
